  ...
  return (
    <nav className="fixed top-6 w-full z-[100] px-6">
      <div className="max-w-7xl mx-auto flex justify-between items-center bg-white/60 dark:bg-gray-900/60 backdrop-blur-xl border border-white/40 dark:border-gray-800/40 px-3 sm:px-6 py-2.5 sm:py-3 rounded-[2rem] shadow-2xl shadow-gray-200/50 dark:shadow-none min-h-[52px] sm:min-h-[64px]">
        ...
        {mobileSearchOpen && (
          <div className="flex sm:hidden items-center gap-3 w-full animate-fade-in">
            <button
              onClick={() => {
                setMobileSearchOpen(false);
              }}
              aria-label="Exit search"
              className="p-2 text-gray-500 hover:text-orange-500 hover:bg-gray-100/60 dark:hover:bg-gray-800/60 rounded-xl transition-colors flex-shrink-0"
            >
            ...
            <div className="flex-1 relative flex items-center">
              ...
              <input
                ...
                autoFocus
                className="w-full bg-gray-100 dark:bg-gray-800 border-2 border-transparent focus:border-orange-500 rounded-xl py-2 pl-10 pr-9 text-sm font-semibold focus:bg-white dark:focus:bg-gray-900 transition-all outline-none text-gray-900 dark:text-white"
              />
              {searchQuery && (
                <button
                  onClick={() => onSearchChange('')}
                  className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-gray-400 hover:text-orange-500 transition-colors"
        ...
        <div className={`${mobileSearchOpen ? 'hidden sm:flex' : 'flex'} w-full items-center justify-between gap-2 sm:gap-4`}>
          {/* Logo - WhosGotStock */}
          <div className="flex-shrink-0">
            ...
            >
              <div className="bg-gray-900 dark:bg-white px-3 sm:px-5 h-8 sm:h-10 rounded-xl sm:rounded-2xl flex items-center justify-center transition-transform group-hover:scale-105 active:scale-95 shadow-lg shadow-gray-200 dark:shadow-none border border-white/10">
                <span className="text-white dark:text-gray-900 font-black text-[10px] sm:text-sm tracking-tighter uppercase whitespace-nowrap">
          ...
          <div className="sm:hidden flex items-center justify-end flex-1 pr-1">
            <button
            ...
            >
              <svg className="w-5.5 h-5.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-4.35-4.35m0 0A7.5 7.5 0 1010.5 3a7.5 7.5 0 006.15 13.65z"/></svg>
            </button>
          </div>
