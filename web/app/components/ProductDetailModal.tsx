            ...
            <div className="relative bg-white dark:bg-gray-900 w-full max-w-5xl max-h-[90vh] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row animate-in zoom-in-95 duration-300 border border-white/20">
                ...
                <div className="w-full md:w-1/2 p-10 overflow-y-auto flex flex-col bg-white dark:bg-gray-900">
                    ...
                    <div className="flex-1 space-y-8">
                        ...
                        <div className="flex flex-wrap gap-4">
                            <div className={`flex items-center gap-3 px-4 py-2 rounded-2xl font-black text-[10px] uppercase tracking-widest ${product.qty_on_hand > 0 ? 'bg-[#D8E698] text-[#4A5D16]' : 'bg-red-50 text-red-600'}`}>
                                ...
                                {product.qty_on_hand > 0 ? `${product.qty_on_hand} Units Total` : 'Currently Unavailable'}
                            </div>

                            {product.stock_jhb !== undefined && product.stock_jhb > 0 && (
                                <div className="px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-2xl text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
                                    <span className="w-1.5 h-1.5 bg-blue-500 rounded-full" />
                                    JHB: {product.stock_jhb}
                                </div>
                            )}

                            {product.stock_cpt !== undefined && product.stock_cpt > 0 && (
                                <div className="px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-2xl text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
                    ...
                    <div className="mt-10 pt-8 border-t border-gray-100 dark:border-gray-800 flex gap-4">
                        ...
                        >
                            <svg className="w-6 h-6 transition-transform group-active:scale-125" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" /></svg>
                        </button>
                    </div>
                </div>
            </div>
